...
import org.terracotta.statistics.TableValueStatistic;
...
public class TableStatisticTest {
  ...
  public void test_returning_live_independent_stats() throws Exception {
    StatisticRegistry registry = new StatisticRegistry();

    // fetching the live stats
    Map<String, Map<String, Supplier<Number>>> liveDbStats = getLiveDbStats();

    // create a "live" table statistic
    ValueStatistic<Table> statistic = TableValueStatistic.newBuilder()
        .withRows(liveDbStats.keySet(), (queryId, rowBuilder) -> rowBuilder
            .registerStatistic("total-exec-count", supply(COUNTER, liveDbStats.get(queryId).get("total-exec-count")))
            .registerStatistic("total-failed-count", supply(COUNTER, liveDbStats.get(queryId).get("total-failed-count"))))
        .build();
  ...
  public void test_returning_dependent_stats_snapshot() throws Exception {
    ...
    assertThat(table.getStatisticNames("prepared-query-1"), hasItems("total-exec-count", "total-failed-count", "total-exec-time"));
    assertThat(table.getStatisticNames("prepared-query-2"), hasItems("total-exec-count", "total-failed-count", "total-exec-time"));
  ...
  private void assertTable(Table table) {
    assertThat(table.getStatisticNames("prepared-query-3"), Matchers.hasSize(0));
    
    assertThat(table.getRowLabels(), hasItems("prepared-query-1", "prepared-query-2"));
    assertThat(table.getStatisticNames("prepared-query-1"), hasItems("total-exec-count", "total-failed-count"));
    assertThat(table.getStatisticNames("prepared-query-2"), hasItems("total-exec-count", "total-failed-count"));

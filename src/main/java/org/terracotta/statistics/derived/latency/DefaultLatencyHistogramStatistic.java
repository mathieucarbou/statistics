...
public class DefaultLatencyHistogramStatistic implements LatencyHistogramStatistic, ChainedEventObserver {
  ...
  // histogram querying without any expire call, except one at the start
  ...
  @Override
  public synchronized List<org.terracotta.statistics.derived.histogram.Histogram.Bucket> buckets() {
  ...
  public synchronized Long percentile(double percent) {
    tryExpire(true, timeSupplier);
    return query.percentile(percent);
  }

  @Override
  public synchronized long[] percentileBounds(double percent) {
    tryExpire(true, timeSupplier);
    return query.percentileBounds(percent);
  }
  ...
  @Override
  public synchronized <T> T multiQuery(Function<Query, T> fn) {
    tryExpire(true, timeSupplier);
